...
describe("Cards component", () => {
  ...
  test("should render 20 cards according to data", async () => {
    const component = render(
      <Cards dataArr={data.results} onCardClick={() => null} />
    );
  ...
  test("should show empty message if no matches", async () => {
    ...
    expect(
      await component.findByText("No matches were found for your search")
    ).toBeVisible();
  });
  ...
  test("should trigger onCardClick function", async () => {
    ...
    const component = render(
      <Cards dataArr={data.results} onCardClick={mockFunc} />
    );
